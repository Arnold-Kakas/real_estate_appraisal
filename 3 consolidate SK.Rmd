...
<br> Prvých/posledných 5 riadkov si zobrazíme pomocou funkcií `head()` resp. `tail()` s parametrom 5.
...
Druhým problémom, ktorý treba bežne riešiť, je prítomnosť <strong>odľahlých hodnôt (outliers)</strong>. Opäť máme niekoľko možných riešení.
...
<br> Je to napríklad stĺpec index a jeho šesť kategórií: environment, quality_of_living, safety, transport, services a relax. Index môže mať hodnotu od 0 do 10 a pripravuje ho slovenský startup [City Performer](https://cityperformer.com/). Tento údaj nie je k dispozícii pre všetky inzeráty. Môžeme však zoskupiť záznamy podľa obce a chýbajúce hodnoty doplniť priemerom. 

```{r}
# price where we have index
mun_ind_price <- joined_data_cleaned %>%
  filter(!is.na(index)) %>%
  group_by(name_nsi) %>%
...
mun_no_ind_price <- joined_data_cleaned %>%
  filter(is.na(index)) %>%
  group_by(name_nsi) %>%
...
mun_imputed_ind_price <- joined_data_cleaned %>%
  filter(!is.na(index2)) %>%
  group_by(name_nsi) %>%
...
mun_imputed_no_ind_price <- joined_data_cleaned %>%
  filter(is.na(index2)) %>%
  group_by(name_nsi) %>%
  summarize(price_with_after = mean(price_after, na.rm = TRUE)) %>% 
  summary() %>% 
...
Podobne ako pri indexe, odľahlé hodnoty cien nehnuteľností budeme odstraňovať v kontexte obcí.
...
# scraped_cleaned_final <- joined_data_cleaned %>%
#   left_join(index_district, by = "district", multiple = "first", keep = FALSE)
```

#### Ostatné faktory

Nasleduje posledné dočistenie dát pred tým, než budeme pokračovať v detailnejšej EDA. V tomto kroku odstraňujeme záznamy z obcí, ktoré majú menej ako 5 inzerátov. 
