...
<br> Komentáre v kódoch ponechávam v angličtine, je to best practice najmä ak pracujete v medzinárodnom prostredí.
...
Druhým dôvodom je, že budem dataset nahrávať na [Kaggle](https://www.kaggle.com).
...
write.csv(joined_data, "data/apartments_appraisal.csv")
```
...
skimmed_summary %>% 
  ...
  kable(format = "html", format.args = list(big.mark = " ")) %>%   kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = F,
                font_size = 12,
                position = "left")
```
...
```{r echo=FALSE}
tail(joined_data_wo_geom, 5) %>% 
  mutate_if(is.numeric,
            round,
            digits = 1) %>% 
  kable(format = "html") %>%   kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = F,
                font_size = 12,
                position = "left") %>%
  scroll_box(width = "100%")
...
-   *Transformácia*: Existuje množstvo metód transformácie dát, ktoré môžeme použit.
    ...
    *Binning* je posledná metóda, ktorú uvediem.
    ...
    Môže sa však nahradiť aj strednou hodnotou v rámci tohto binu (napr. priemerom 33,2 alebo medianom 32,9).
...
Následne môžeme vyriesiť tie prípady, kde imputácia dáva zmysel.
...
```{r}
quantile(joined_data_cleaned$area, probs = seq(.95, 1, by = .01), na.rm = TRUE) %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  `colnames<-`(c("Percentil", "Rozloha")) %>% 
  kable(format = "html", digits = 1, format.args = list(big.mark = " ")) %>% 
...
Namiesto 5 by sme mohli vzbrať aj iné číslo, ale existuje "rule of thumb", ktoré tvrdí, že je 93,75% šanca, že medián populácie is medzi najnižšou a najvyššou hodnotou náhodne vybranej vzorky.
...
Po týtcho pár zmenách je stav viditeľne lepší.
...
price1 <- joined_data_cleaned %>% 
  ...
  labs(y = "Počet",
       x = NULL,
       title = "Rozloženie cien nehnuteľností")


price2 <- joined_data_cleaned %>% 
  ggplot(aes(y = price)) +
...
plot_grid(price1, price2, ncol = 1, align = "v")
```
...
index1 <- joined_data_cleaned %>% 
  ...
  labs(y = "Počet",
       x = NULL,
       title = "Rozloženie indexu bývania")

index2 <- joined_data_cleaned %>% 
  select(index) %>% 
  drop_na() %>%
  ggplot(aes(y = index)) +
  geom_boxplot(col = 'steelblue', outlier.alpha = 0.1) + 
...
plot_grid(index1, index2, ncol = 1, align = "v")
```
...
index_categories_df <- index_categories_df %>% 
  pivot_longer(cols = c(environment, quality_of_living, safety, transport, services, relax),
                 names_to = "Category",
               values_to = "value") %>% 
...
```{r echo=FALSE}
categories_boxplot <- index_categories_df %>% 
  ggplot(aes(y = value, color = factor(Category,
    levels = c('environment', 'quality_of_living', 'safety', 'transport', 'services', 'relax'),ordered = TRUE))) +
...
```{r, echo=TRUE, message=FALSE, warning=FALSE}
area1 <- joined_data_cleaned %>% 
  drop_na() %>% 
  ggplot(aes(x = area, fill = type)) +
        geom_histogram() +
  theme_minimal() +
  labs(title = "Rozloženie rozlohy nehnuteľností",
       x = NULL,
       y = "Počet",
       fill = "Typ")

area2 <- joined_data_cleaned %>% 
  drop_na() %>% 
  ggplot(aes(y = area)) +
  geom_boxplot(col = 'steelblue', outlier.alpha = 0.1) + 
  theme_minimal() +
...
plot_grid(area1, area2, ncol = 1, align = "v")
```

<br> Podobne ako pri cene, aj teraz môžeme vidiet rozloženie hodnôt s pozitívnym (pravostranným) zošikmením.
...
<br> Energetický certifikát hodnotí budovy z hľadiska 4 bodov spotreby energie:
...
<br> 59% inzerovaných bytov nemá uvedený energetický certifikát. Skoro 24% má explicitne uvedené, že takýto certifikát nemá. Najhoršie certifikáty D až F sú zastúpené veľmi málo. 
<br>

```{r, echo=TRUE}
cert1 <- joined_data_cleaned %>%
...
top_10_districts_chart <- top_bottom_10_districts %>% 
  ggplot(aes(x = reorder(district, -price), y = price, text = paste0("Priemerná cena: ",
...
-   Existujú preukázané rozdiely medzi cenami bytov s rôznymi stavmi. Nie je prekvapujúce, že nové byty majú najvyššie ceny. Existuje viac ako 800 NA, ktoré je potrebné pripočítať.
- Tiež certifikát preukázal vplyv, v tomto prípade je problém s NAS vážnejší, pretože viac ako 7 500 inzerátov neobsahuje tieto informácie. Stále budem imputovať túto premennú, pretože predpokladám, že existuje spojenie medzi certifikátom a podmienkou
- Veľká väčšina bytov v súbore údajov má 2 a 3 izby. Cena rastie s rastúcou veľkostnou skupinou. Zvýšenie ceny z 2 izbovej na 3 izbovú skupinu je v priemere dosť nízke. Vidím dva možné dôvody - dopyt po 2 izbových bytoch (keďže sú stále lacnejšie ako 3 izbové) a lokalita. Ak by sa väčšina 2-izbových bytov nachádzala na západe, ich cena by bola v priemere za celú krajinu vyššia v porovnaní s rovnomerným priestorovým rozložením.
