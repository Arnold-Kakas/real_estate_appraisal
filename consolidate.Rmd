...
scraped_data <- readRDS("data/advertisements_complete_geocoded.RDS") %>% 
  ...
  select(-link, -info_text, -address, -address1, -address2) %>% 
...
```{r}
rm(list = setdiff(ls(), "scraped_cleaned_wo_outliers"))
...
scraped_cleaned_wo_outliers <- scraped_cleaned_wo_outliers %>% st_centroid()
  
advertisements_complete_districts_geocoded <- districts %>% 
  st_join(scraped_cleaned_wo_outliers, join = st_contains) %>% 
...
```{r}
st_geometry(apartments_final_data) <- NULL
