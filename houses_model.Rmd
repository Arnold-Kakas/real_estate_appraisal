...
p_load(rio, tidyverse, tidymodels, GGally, mice, vip, reticulate, skimr)
...
```{python}
# Translate a given text to English using the google_trans_new module
...
def translate_text(text):
    translator = gtn.google_translator(timeout=5).translate("en")
    return translator.translate(text)

# Translate the text in the given dataframe and create a new column with the translated text
    
#    Parameters:
#    df (pandas.DataFrame): Dataframe to be translated
#    text_vector (str): Column name in the dataframe to be translated

#    Returns:
#    pandas.DataFrame: Translated dataframe with a new column for the translated text

def translate_df(df, text_vector):
    pool = mpd.Pool(6)
    translated_text = pool.map(translate_text, df[text_vector])
    df['translated_text'] = translated_text
    return df
```

```{python}
houses_translated = translate_df(r.houses_cleaned, 'info_text')
```
